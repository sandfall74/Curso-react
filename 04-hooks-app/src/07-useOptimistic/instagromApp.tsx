...
export const InstagromApp = () => {
  ...
  const [optimisticComments,addOptimisticCommnet]= useOptimistic(comments,(currentComments,newComment:string)=>{
    lastId++;
  ...
  const handleAddComment = async (e:FormData) => {
    const messageText=e.get('post-message') as string;
    addOptimisticCommnet(messageText);
    statTransition(async()=>{
        await new Promise(resolve=>setTimeout(resolve,3000));
    /* setComments(prevs=>[...prevs,{
        id:lastId,
        text:messageText
    }])*/
